...
export async function playQuery(client, interaction, query) {
    ...
    try {
        ...
        logger?.warn?.('[MUSIC] Riffy resolve failed', { error: err?.message || err });
        throw createError(
            'Music resolve failed',
            ErrorTypes.NETWORK,
            'Failed to load the requested track. Please try again in a moment.',
            { errorCode: 'MUSIC.RESOLVE_FAILED' }
        );
    }
